...
<nav class="bg-gray-800 pt-2 md:pt-1 pb-1 px-1 mt-0 h-auto fixed w-full z-20 top-0">
    ...
    <div class="flex flex-wrap items-center">
        ...
        <div class="flex flex-1 md:w-1/3 justify-center md:justify-start text-white px-2">
            <span class=""> Wellcome back to Moscow Courier ! </span>
        </div>
    ...
    <div class="bg-gray-800 shadow-xl h-16 fixed bottom-0 mt-12 md:relative md:h-screen z-10 w-full md:w-48">
        ...
        <div
            ...
            <ul class="list-reset flex flex-row md:flex-col py-0 md:py-3 px-1 md:px-2 text-center md:text-left">
                ...
                <li class="mr-3 flex-1">
                    <a href="{{ '/driver/tasks'}}"
                        class="block py-1 md:py-3 pl-1 align-middle text-white no-underline hover:text-white border-b-2 {{ request()->is('driver/tasks') ? 'border-blue-600' : 'border-gray-800' }} hover:border-purple-500">
                        <i class="fa fa-tasks pr-0 md:pr-3"></i><span
                            class="pb-1 md:pb-0 text-xs md:text-base {{ request()->is('tasks') ? 'text-white' : 'text-gray-600 md:text-gray-400' }} block md:inline-block">Current
                            Tasks
                        </span>
                    </a>
                </li>
                <li class="mr-3 flex-1">
                    <a href="{{ route('completedTasks') }}"
                        class="block py-1 md:py-3 pl-0 md:pl-1 align-middle text-white no-underline hover:text-white border-b-2 {{ request()->is('driver/all-task') ? 'border-blue-600' : 'border-gray-800' }} hover:border-red-500">
                        <i class="fa fa-clipboard-list pr-0 md:pr-3"></i><span
                            class="pb-1 md:pb-0 text-xs md:text-base {{ request()->is('taskscompleted') ? 'text-white' : 'text-gray-600 md:text-gray-400' }} block md:inline-block">All
                            Tasks</span>
                    </a>
                </li>
                <li class="mr-3 flex-1">
        ...
        <div class="bg-gray-800 pt-3">
            <div class="rounded-tl-3xl bg-gradient-to-r from-blue-900 to-gray-800 p-4 shadow text-2xl text-white">
                <h3 class="font-bold pl-2"> {{ Auth::user()->name }},
                    @if (request()->is('driver/tasks'))
                    Your current/Ongoing tasks!
                    @elseif (request()->is('dashboard'))
                    Hi
                    @elseif (request()->is('driver/all-task'))
                    Your all tasks!
                    @endif
                </h3>
            </div>
